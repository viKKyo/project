...
class SuperHaxxorForensicTool(tkinter.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.frame = {}
        for F in (
        StartPage, PageOne, PageTwo, PageThree, PageThreeOne, PageThreetwo,
...
class StartPage(tkinter.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.button8 = tkinter.Button(self, text="F8 Systeminfo", fg='black', bg='green',
                                      command=lambda: controller.show_frame(PageEight))
        self.button9 = tkinter.Button(self, text="F9 Hash", fg='black', bg='green',
                                      command=lambda: controller.show_frame(PageNine))

        self.button1.grid(row=1, column=2, columnspan=1, sticky='EWNS')
        self.button2.grid(row=2, column=2, columnspan=1, sticky='EWNS')
        self.button3.grid(row=3, column=2, columnspan=1, sticky='EWNS')
        self.button5.grid(row=4, column=2, columnspan=1, sticky='EWNS')
        self.button6.grid(row=5, column=2, columnspan=1, sticky='EWNS')
        self.button7.grid(row=6, column=2, columnspan=1, sticky='EWNS')

        self.label2 = tkinter.Label(self, text="Extrauppgifter", font="Times 12 bold italic")
        self.label2.grid(row=9, column=2, columnspan=2, sticky='EWNS')
        self.button8.grid(row=10, column=2, columnspan=1, sticky='EWNS')
        self.button9.grid(row=11, column=2, columnspan=1, sticky='EWNS')
...
class PageTwo(tkinter.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.label = tkinter.Label(self, text="Creates a list of specific file-types", fg='red',
...
class PageThreetwo(tkinter.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.label.grid(row=0, column=1, columnspan=1, sticky='EWNS')
        self.label1.grid(row=2, column=1, columnspan=1, sticky='EWNS')
        self.entry.grid(row=2, column=2, columnspan=1, sticky='EWNS')
        self.label_search.grid(row=1, column=1, columnspan=1, sticky='EWNS')
        self.entry_searchword.grid(row=1, column=2, columnspan=1, sticky='EWNS')
        self.button_search.grid(row=3, column=2, columnspan=1, sticky='EWNS')
        self.button2.grid(row=4, column=2, columnspan=1, sticky='EWNS')
...
for i in range(len(KRYP_LISTA)):
    kryp_dic[KRYP_LISTA[i]] = KRYP_LISTA[-i - 1]
...
class PageEight(tkinter.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.label = tkinter.Label(self, text="Save Systeminfo to different files.", fg='red',
                                   font="Times 16 bold italic")
        self.label.grid(row=0, column=2, columnspan=1, sticky="EWNS")
...
# Extra
# page9
class PageNine(tkinter.Frame):


    def __init__(self, parent, controller):
        tkinter.Frame.__init__(self, parent)

        self.label = tkinter.Label(self, text="Funktion 9 Check Hash", fg='red', font="helvetica 16 bold italic")

        self.label2 = tkinter.Label(self, text='input the full file path for the file you want to check')
        self.label3 = tkinter.Label(self, text='input the file you want to blacklist')

        self.button2 = tkinter.Button(self, text='check file',
                                      command = self.check_file_hash)
        self.button3 = tkinter.Button(self, text='add filehash to blacklist.txt',
                                      command= self.blacklist_file)
        self.button4 = tkinter.Button(self, text="Go to the start page",
                                      command=lambda: controller.show_frame(StartPage))

        self.entry2 = tkinter.Entry(self, width='40')
        self.entry3 = tkinter.Entry(self, width='40')

        self.label.grid(row=1, column=1, columnspan=1, sticky='ESWN')
        self.label2.grid(row=2, column=1, columnspan=1, sticky='ESWN')
        self.label3.grid(row=4, column=1, columnspan=1, sticky='ESWN')

        self.entry2.grid(row=2, column=2, columnspan=1, sticky='ESWN')
        self.entry3.grid(row=4, column=2, columnspan=1, sticky='ESWN')

        self.button2.grid(row=3, column=2, columnspan=1, sticky='ESWN')
        self.button3.grid(row=5, column=2, columnspan=1, sticky='ESWN')
        self.button4.grid(row=6, column=1, columnspan=2, sticky="EWNS")


    def blacklist_file(self):

        # open file to append
        sus_file = self.entry3.get()
        blacklist = open("blacklist.txt", "a")
        hashfile = open(sus_file, "r", errors="ignore")
        hashstring = ""

        for x in hashfile:
            hashstring = hashstring + x

        # write the hash to blacklist file
        sha = hashlib.sha256(hashstring.encode())
        hashed = sha.hexdigest()
        blacklist.write(hashed + "\n")
        messagebox.showinfo('results', "file added to blacklist")
        print("File added to blacklist.")


    def check_file_hash(self):

        # open file to read
        file_path = self.entry2.get()
        blacklist = open("blacklist.txt", "r")
        hashit = open(file_path, "r", errors="ignore")
        hashstring = ""
        ...
        # check if found in blacklist.txt
        black_string = ""
        for z in blacklist:
            black_string += z
        black_list = black_string.split("\n")

        if hashed in black_list:
            messagebox.showinfo('results', "filename found in blacklist")
            print("File found in blacklist!")
        elif hashed not in black_list:
            messagebox.showinfo('results', "filename not found in blacklist")
            print("File not found in blacklist.")
